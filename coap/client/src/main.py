...
while True:
    client = HelperClient(server=(#TODO instantiate a CoAP client by specifying host and port))
    data_as_hex = iot_data.get_data_as_hex(pressure, temperature, volume)
    #TODO send data with post method by specifying path and payload (add option no_response=True)
    client.stop()
    time.sleep(10)
